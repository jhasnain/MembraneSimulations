...
int GlobalMeshShift(sys *Sys){
  ...
  double deltaMeshParticle=0.0, deltaEParticle = 0.0, MyIntStr, l_partner_on2, l_current_on2, cutoffsqr;
  
  
  for(pBot=0;pBot<Sys->TotCurPop[0];pBot++){
    CurrentParticle = Sys->Particles[pBot];
    CurrentParticle.Pos.z -=suggestedz;
    deltaMeshParticle+=ParticleMeshEnergy(Sys, &CurrentParticle);
    CurrentParticle.Pos.z +=suggestedz;
    if(deltaMeshParticle>=ALOT){return 0;}
  }
      ...
      for(ibot = 0;ibot<CellList->BotPopulations[CxBot][CyBot];ibot++){
        ...
        for(popind=0;popind<5;popind++){
          ...
          for(itop = 0;itop<CellList->TopPopulations[CellNeighx][CellNeighy];itop++){
            ...
            if( MyIntStr>1e-10 ){
              deltaEParticle += CheckForBond(CurrentParticle, PartnerParticle, Lx, Ly, Sys);
              deltaEParticle -= CheckForBond(CurrentParticle, PartnerParticle, Lx, Ly, Sys);
            }
          }
        }
  ...
  if ( gsl_rng_uniform(Sys->rng)< exp(-(deltaEParticle+DeltaEBias)) ){
    ...
    for(int nx=0; nx<Sys->Nx; nx++){
      for(int ny=0; ny<Sys->Ny; ny++){
        Sys->TopMesh[nx][ny].Pos.z+=suggestedz;
      }
    }
    for(int p=Sys->maxpop ;p<Sys->maxpop + Sys->TotCurPop[1];p++){
...
void MeshParticleChange(sys *Sys, double *Energies){
  ...
  double l_moved_on2, l_partner_on2, cutoffsqr, MyIntStr;
  ...
  for(i_affect=0;i_affect<Sys->AffectedParticleNum;i_affect++){
    ...
    l_moved_on2 = current_Particle.MyLength/2;
    MyIntStr=current_Particle.MyIntStr;
    ...
    for(i_cellNeigh=0;i_cellNeigh<9;i_cellNeigh++){
      ...
      for(i_cellpop = 0;i_cellpop<CellList->TopPopulations[Cx][Cy];i_cellpop++){
        ...
        if(current_p != i_partner){
          ...
          if(flag>0){
            l_partner_on2 = PartnerParticleNew.MyLength/2;
            cutoffsqr= (l_partner_on2 + l_moved_on2)*(l_partner_on2 + l_moved_on2);
            if( deltaRsqr(current_Particle.MidPoint, PartnerParticleNew.MidPoint, Lx, Ly) <  cutoffsqr){
              if ( CheckOverlap(current_Particle, PartnerParticleNew, Lx, Ly, &tmp) == ALOT){Energies[1]=ALOT;break;}
            }
            
            if( MyIntStr>1e-10 ){
              Energies[1]+= CheckForBond(current_Particle         , PartnerParticleNew, Lx, Ly, Sys)
                           -CheckForBond(Sys->Particles[current_p], PartnerParticleOld, Lx, Ly, Sys);
            }
          } 
        }
      }
...
double ComputePMChange(sys *Sys){
  ...
  double dRsqr, diffleft, diffright, diffup, diffdown, z1, z2, z3, z4;
  ...
  for(int k=0;k<9;k++){
    ...
    for(p=0;p<MyPopulations[Cx][Cy];p++){
      ...
      if ( (diffleft < 0.0 && diffright > 0.0) && (diffdown < 0.0 && diffup > 0.0 ) ) {
        ...
        twodvecdiff(&(diffvec), MyPos, MyMesh[inew][jnew].Pos);
        ...
        dRsqr = threed_NormSqr(diffvec);

        if( Sys->Particles[particleId].MeshId*diffvec.z < 0.0){
          Sys->dummy_particle.MeshParticleEnergy = ALOT;
          DeltaEMeshParticle+=ALOT - Sys->Particles[particleId].MeshParticleEnergy;
        }
        else if( dRsqr < Sys->Particles[particleId].MeshParticleCutoffsqr ){
          Sys->dummy_particle.MeshParticleEnergy = MeshParticleInteraction(dRsqr);
          DeltaEMeshParticle+=Sys->dummy_particle.MeshParticleEnergy - Sys->Particles[particleId].MeshParticleEnergy;
          Sys->p0 = particleId;
        }
      }
    }
