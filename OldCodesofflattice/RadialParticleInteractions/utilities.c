...
void PrintConfiguration(FILE *fp, sys *Sys, const int opt){
  ...
  for(mytype = 0;mytype<Sys->NumPTypes;mytype++){
    mylength = opt*(Sys->vmdLength[mytype] - 1) + 1;
//     printf("%d %d\n", Sys->TotCurPop[0], Sys->TotCurPop[1]);
      ...
      if(Sys->Particles[j].type == mytype ){
        for(int k=0;k<mylength;k++){
          fprintf(
            ...
            AtomNames[mytype+1],
            Sys->Particles[j].Pos.x + k*Sys->DPType[mytype]*Sys->Particles[j].nhat.x,  
            Sys->Particles[j].Pos.y + k*Sys->DPType[mytype]*Sys->Particles[j].nhat.y,  
            Sys->Particles[j].Pos.z + k*Sys->DPType[mytype]*Sys->Particles[j].nhat.z,  
    ...
    for(j=0;j<Sys->twomaxpop - (Sys->CurParPop[0][mytype] + Sys->CurParPop[1][mytype]);j++){
      for(int k=0;k<mylength;k++){
        fprintf(
          fp, "%s %lf %lf %lf %lf %lf %lf\n", 
...
//   for (i=0;i<Sys->twomaxpop;i++){
//     if(i == Sys->TotCurPop[0]) i=Sys->maxpop;
//     if(i == Sys->TotCurPop[1] + Sys->maxpop) break;
//     
//     mytype = Sys->Particles[i].type;
//     mylength = opt*(Sys->vmdLength[mytype] - 1) + 1;
//     for(j=0;j<mylength;j++){
//       fprintf(
//         fp, "%s %lf %lf %lf %lf %lf %lf\n", 
//         AtomNames[mytype+1],
//         Sys->Particles[i].Pos.x + j*Sys->DPType[mytype]*Sys->Particles[i].nhat.x,  
//         Sys->Particles[i].Pos.y + j*Sys->DPType[mytype]*Sys->Particles[i].nhat.y,  
//         Sys->Particles[i].Pos.z + j*Sys->DPType[mytype]*Sys->Particles[i].nhat.z,  
//         Sys->Particles[i].nhat.x,
//         Sys->Particles[i].nhat.y,
//         Sys->Particles[i].nhat.z
//       );
//     }
//   }
}

void PrintData(FILE *fp, sys *Sys, int currentMCstep){
  int i, shift = 14;
  
  if (currentMCstep==0) {
    fprintf(fp, "#Sweep(1) TAcc(2) PAcc(3) MAcc(4) GCAcc(5) TotE(6) MeshE(7) BiasE(8) PartE(9) PhaseE(10) PartMeshE(11) ThetaVal(12) TotPop(13) TotBotPop(14) ");
  ...
  fprintf( fp, "%ld %.2lf %.2lf %.2lf %.2lf %lf %lf %lf %lf %lf %lf %lf ", 
    Sys->SweepIndex, 
    ...
    Sys->TotEnergy, Sys->MeshEnergy, Sys->BiasEnergy, Sys->TotParticleEnergy, Sys->TotPhaseFactorEnergy, Sys->TotMeshParticleEnergy, Sys->MeshBias.theta
  );
...
void PrintDataToScreen(FILE *fp, sys *Sys, int currentMCstep){
