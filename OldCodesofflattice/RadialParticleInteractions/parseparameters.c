...
//   snprintf(arguments->PTypePop,  1024, "10 10");
//   snprintf(arguments->LPType,    1024, "16 8");
//   snprintf(arguments->DPType,    1024, "4 4");
//   snprintf(arguments->BindPType, 1024, "0.0 1.0");
//   snprintf(arguments->RanPType,  1024, "4.0 4.0");
//   snprintf(arguments->ChemPots,  1024, "0.0 0.0 0.0 0.0");
  
  arguments->NumPType=1;
  snprintf(arguments->PTypePop,  1024, "10");
  snprintf(arguments->LPType,    1024, "12.0");
  snprintf(arguments->DPType,    1024, "4.0");
...
//MC properties
  ...
  arguments->theta0=60.0;
  snprintf(arguments->BiasType, 1024, "Harmonic");
  arguments->MCSweeps=100;
  ...
  for(mytype = 0;mytype<Sys->NumPTypes;mytype++){
    for(j = 0;j<Sys->TotCurPop[0];j++){
      if(Sys->Particles[j].type == mytype ){
        fprintf(
          fp, "%d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", 
          ...
          Sys->Particles[j].MyIntCutoffsqr
        );
      }   
    }
  }
...
void ReadConfiguration(FILE *fp, sys *Sys){
  ...
  for (i=0;( i<Sys->maxpop && (strcmp(line, "TopConfig:\n") != 0) );i++){
    sscanf(line, 
      "%d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", 
      ...
      &(Sys->Particles[i].MyIntCutoffsqr)
    );

    length = Sys->Particles[i].MyLength;
