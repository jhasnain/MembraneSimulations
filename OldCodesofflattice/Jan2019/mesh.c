...
void PopulateAffectedParticles(sys *Sys){
  ...
  double threedx = 3.0*Sys->dx, threedy = 3.0*Sys->dy;
  ...
  for(i=0;i<9;i++){
    ...
    for(k=0;k<Populations[Cx][Cy];k++){
      ...
      NearestImageConv(&diffvec, Sys->Lx, Sys->Ly);
      if( fabs(diffvec.x)<threedx && fabs(diffvec.y)<threedy ){
...
//   for(i=0;i<Sys->AffectedParticleNum;i++){
//     p=Sys->Unique_dummy_index[i];
//     for(k=i+1;k<Sys->AffectedParticleNum;k++){
//       if(Sys->Unique_dummy_index[i]==Sys->Unique_dummy_index[k])
//       printf("Duplicate Ids!: %d %d\n", Sys->Unique_dummy_index[i], Sys->Unique_dummy_index[k]);
//     }
//   }
}
