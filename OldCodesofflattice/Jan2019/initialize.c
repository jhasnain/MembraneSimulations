...
void InitializeParticleProperties(sys *Sys, struct arguments *arguments){
  ...
  double x = Sys->Lx/2, y = Sys->Ly/2;
  
  double delta = Sys->DPType[0][type];
  double deltax = 1.1*Sys->dx*ceil(delta / Sys->dx);
  double deltay = 1.1*Sys->dy*ceil(delta / Sys->dy);
  ...
  for(p=0;p<Sys->maxpop;p++){
    if(p >= Sys->TotCurPop[0] ){
      ...
      Sys->Particles[p].Pos.z = -arguments->z0/2;
      ...
      Sys->Particles[p].MyDiam   = Sys->DPType[0][type];
      Sys->Particles[p].MyRad    = Sys->Particles[p].MyDiam/2;
      Sys->Particles[p].MyRadSqr = Sys->Particles[p].MyRad*Sys->Particles[p].MyRad;
      Sys->Particles[p].MyLength = Sys->LPType[0][type];
      Sys->Particles[p].MeshParticleCutoffsqr =  Sys->Particles[p].MyRadSqr;
      ...
      x+=deltax;
      if(x + deltax/2.0 > Sys->Lx){
        x=0.0;y+=deltay;
        if(y + deltay/2.0 > Sys->Ly) y=0.0;
      }
        
      if(p==partialPSum-1){
        type++;
        partialPSum+=Sys->CurParPop[0][type];
        delta = Sys->DPType[0][type];
  ...
  x=Sys->Lx/2;y=Sys->Ly/2;
  delta = Sys->DPType[1][type];
  deltax = 1.1*Sys->dx*ceil(delta / Sys->dx);
  deltay = 1.1*Sys->dy*ceil(delta / Sys->dy);
  ...
  for(p=Sys->maxpop;p<Sys->twomaxpop;p++){
    if(p - Sys->maxpop >= Sys->TotCurPop[1]){
      ...
      Sys->Particles[p].MyDiam   = Sys->DPType[1][type];
      Sys->Particles[p].MyRad    = Sys->Particles[p].MyDiam/2;
      Sys->Particles[p].MyRadSqr = Sys->Particles[p].MyRad*Sys->Particles[p].MyRad;
      Sys->Particles[p].MyLength = Sys->LPType[1][type];
      Sys->Particles[p].MeshParticleCutoffsqr =  Sys->Particles[p].MyRadSqr;
...
void CreateCellLists(sys *Sys){
  ...
  double maxlength = -1000.0, minrad = 10000, maxdiam=-10000;
  for(k=0;k<Sys->NumPTypes;k++){
    maxlength=max(maxlength, 1.1*Sys->LPType[0][k]);
    maxlength=max(maxlength, 1.1*Sys->LPType[1][k]);
    maxdiam  =max(maxdiam  , 1.1*Sys->DPType[0][k]);
    maxdiam  =max(maxdiam  , 1.1*Sys->DPType[1][k]);
    minrad=min(minrad, Sys->DPType[0][k]/2);
    minrad=min(minrad, Sys->DPType[1][k]/2);
  }
  
  double cutoff = max(maxdiam, max(maxlength, 2.1*Sys->dx));
...
void CreateParticles(sys *Sys, struct arguments *arguments){
  ...
  if (arguments->NumPType>0){
    ...
    Sys->AccessVol[0] = (double *)malloc(pflavors*sizeof(double));
    Sys->AccessVol[1] = (double *)malloc(pflavors*sizeof(double));
    ...
    Sys->vmdLength[0] = (int *)malloc(pflavors*sizeof(int));
    Sys->vmdLength[1] = (int *)malloc(pflavors*sizeof(int));
    ...
    for(i=0;i<pflavors;i++){
      minrad    = min(minrad, Sys->DPType[0][i]/2);
      minrad    = min(minrad, Sys->DPType[1][i]/2);
      maxlength = min(maxlength, Sys->LPType[0][i]);
      maxlength = min(maxlength, Sys->LPType[1][i]);
    }
    ...
    for(i=0;i<pflavors;i++){
      Sys->vmdLength[0][i]= (int)floor(Sys->LPType[0][i]/Sys->DPType[0][i]) + 1;
      Sys->vmdLength[1][i]= (int)floor(Sys->LPType[1][i]/Sys->DPType[1][i]) + 1;
      
      Sys->vmdDoF+= Sys->maxpop*Sys->vmdLength[0][i] + Sys->maxpop*Sys->vmdLength[1][i];
      ...
      Sys->AccessVol[0][i] = Sys->A/(M_PI*(Sys->DPType[0][i]/2)*(Sys->DPType[0][i]/2));
      Sys->AccessVol[1][i] = Sys->A/(M_PI*(Sys->DPType[1][i]/2)*(Sys->DPType[1][i]/2));
    }
