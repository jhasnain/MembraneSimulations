...
double SuggestParticleInsertion(sys *Sys){
  ...
  Sys->dummy_particle.MyDiam   = Sys->DPType[meshindex][PType];
  ...
  Sys->dummy_particle.MyLength = Sys->LPType[meshindex][PType];
  ...
  Sys->dummy_particle.MeshParticleCutoffsqr = Sys->dummy_particle.MyRadSqr;
  
  Sys->p0 = Sys->TotCurPop[meshindex] + Sys->maxpop*meshindex;
...
double GCMCAdditionProb(sys *Sys, double DeltaETot){
 ...
 return Sys->AccessVol[MeshIndex][TypeIndex]/(Sys->CurParPop[MeshIndex][TypeIndex] + 1)*exp(Sys->ChemPots[MeshIndex][TypeIndex] - DeltaETot);
}

double GCMCRemovalProb(sys *Sys, double DeltaETot){
 int MeshIndex = (Sys->GC_TopOrBot == BOT) ? 0 : 1;
 int TypeIndex = Sys->GC_PType; 
 return (Sys->CurParPop[MeshIndex][TypeIndex]/(double)Sys->AccessVol[MeshIndex][TypeIndex])*exp(-Sys->ChemPots[MeshIndex][TypeIndex] - DeltaETot);
}
...
void ReplaceParticle(sys *Sys, double DeltaEPhase, double DeltaEPart){
  ...
  Sys->Particles[replaceindex].type = EMPTY;
}
...
double PerformGCMCMove(sys *Sys){
  ...
  if (Sys->GC_AddRemove == 0){
    ...
    if(DeltaEPhase < ALOT){
      double DeltaEPart = BondGCMC(Sys);
...
//         printf("\nAcc del %d %lf %d\n", Sys->p0, DeltaEPart, Sys->BondList.ParticleBonds[Sys->p0].currbond);
        UpdateBondLists(Sys, 3);
