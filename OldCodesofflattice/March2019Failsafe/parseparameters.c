...
/* Get the input argument from argp_parse, which we
  ...
  switch (key)
    ...
    case 't':strcpy(arguments->Tag,  arg);break;
    case 'u':sscanf(arg, "%d",  &arguments->framenums);break;
    case 'v':sscanf(arg, "%d",  &arguments->datnums);break;
    
    case 'w':strcpy(arguments->Inputfile, arg);break;
    case 'x':strcpy(arguments->Equil,     arg);break;
    
    case 'y':sscanf(arg, "%d",  &arguments->MCSweeps);break;
    
    case 'z':sscanf(arg, "%lf", &arguments->BiasStr);break;
    case '1':strcpy(arguments->BiasType, arg);break;
    case '2':sscanf(arg, "%lf", &arguments->theta0);break;
    
    case '3':strcpy(arguments->ErrorCheck,  arg);break;
    case '4':sscanf(arg, "%ld", &arguments->RandSeed);break;
...
//   arguments->NumPType=0;
  ...
  snprintf(arguments->PTypePop,  1024, "0");
  snprintf(arguments->LPType,    1024, "5.0 10.0");
  snprintf(arguments->DPType,    1024, "4.0 4.0");
  snprintf(arguments->BindPType, 1024, "8.0");
  snprintf(arguments->RanPType,  1024, "4.0");
  snprintf(arguments->ChemPots,  1024, "-5.5 -7.3");

//   arguments->NumPType=2;
//   snprintf(arguments->PTypePop,  1024, "0 0");
//   snprintf(arguments->LPType,    1024, "8.0 12.0 8.0 5.0");
//   snprintf(arguments->DPType,    1024, "4.0 4.0 4.0 4.0");
//   snprintf(arguments->BindPType, 1024, "8.0 0.0");
//   snprintf(arguments->RanPType,  1024, "4.5 0.5");
//   snprintf(arguments->ChemPots,  1024, "-4.0 -4.0 -4.0 -4.0");
  
//   arguments->NumPType=3;
//   snprintf(arguments->PTypePop,  1024, "0 0 0");
//   snprintf(arguments->LPType,    1024, "8.0 16.0 24.0 8.0 16.0 24.0");
//   snprintf(arguments->DPType,    1024, "4.0 4.0 4.0 4.0 4.0 4.0");
...
//MC properties
  arguments->MRate =0.9;
  arguments->PRate =0.05;
  arguments->GCRate=0.05;
  
  arguments->MDisp=1.0;
  arguments->PDisp=10.0; 
  
  arguments->z0=15.0;
  arguments->BiasStr=1.0;
  arguments->theta0=15.0;
...
void ConfigurationRestart(FILE *fp, sys *Sys){
  ...
  for(j = 0;j<Sys->TotCurPop[0];j++){
    fprintf(
      ...
      (Sys->BondList.BondableTypes>0 ? Sys->BondList.ParticleBonds[j].currbond:-1)
    );
  }
...
void ReadConfiguration(FILE *fp, sys *Sys){
  ...
  for (i=0;( i<Sys->maxpop && (strcmp(line, "TopConfig:\n") != 0) );i++){
    sscanf(line, 
      "%d %d %lf %lf %lf %lf %lf %lf %d\n", 
      &(dummytype),     &(Sys->Particles[i].MeshId),
      ...
      &(Sys->BondList.ParticleBonds[i].currbond) 
    );

    Sys->Particles[i].type           = dummytype;
    Sys->Particles[i].MyDiam         = Sys->DPType[0][dummytype];
    Sys->Particles[i].MyRad          = Sys->DPType[0][dummytype]/2;
    Sys->Particles[i].MyRadSqr       = Sys->Particles[i].MyRad*Sys->Particles[i].MyRad;
    Sys->Particles[i].MyLength       = Sys->LPType[0][dummytype];
    Sys->Particles[i].MyIntStr       = Sys->BindPType[dummytype];
    Sys->Particles[i].MyIntCutoff    = Sys->RanPType[dummytype];
    Sys->Particles[i].MyIntCutoffsqr = Sys->Particles[i].MyIntCutoff*Sys->Particles[i].MyIntCutoff;
  ...
  for (i=Sys->maxpop;(i<Sys->twomaxpop && (dummy != NULL));i++){
    ...
    Sys->Particles[i].type           = dummytype;
    Sys->Particles[i].MyDiam         = Sys->DPType[1][dummytype];
    Sys->Particles[i].MyRad          = Sys->DPType[1][dummytype]/2;
    Sys->Particles[i].MyRadSqr       = Sys->Particles[i].MyRad*Sys->Particles[i].MyRad;
    Sys->Particles[i].MyLength       = Sys->LPType[1][dummytype];
    Sys->Particles[i].MyIntStr       = Sys->BindPType[dummytype];
    Sys->Particles[i].MyIntCutoff    = Sys->RanPType[dummytype];
    Sys->Particles[i].MyIntCutoffsqr = Sys->Particles[i].MyIntCutoff*Sys->Particles[i].MyIntCutoff;
