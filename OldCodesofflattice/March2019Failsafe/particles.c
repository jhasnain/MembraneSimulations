...
int ErrorCheck_Z_nhat_PhaseFactor(sys *Sys){
  ...
  for(i=0;i<2;i++){
    for(p=i*Sys->maxpop;p<Sys->TotCurPop[i]+i*(Sys->maxpop);p++){
      ...
      if(dotprod(testnhat, Sys->Particles[p].nhat)<1.0-1e-4 ){
        printf("\nNhat Error!\n");Err=2;
        printf("Id: %d\n", p);
        PrintVec(testnhat,"computed orientation");
        PrintVec(Sys->Particles[p].nhat,"particle orientation");
        printf("Nodes used %d %d %d %d\n", inew, jnew, xNeigh, yNeigh);
        
        printf("dotprod %lf\n", dotprod(testnhat, Sys->Particles[p].nhat));
      }
      threedvecsumscalar( &(testnhat), Sys->Particles[p].Pos, Sys->Particles[p].nhat, Sys->Particles[p].MyLength/2);
      if(fabs(testnhat.x -Sys->Particles[p].MidPoint.x)>1e-3 || fabs(testnhat.y -Sys->Particles[p].MidPoint.y)>1e-3 || fabs(testnhat.z -Sys->Particles[p].MidPoint.z)>1e-3){
        printf("\nMidpoint Error!\n");Err=2;
        printf("Id: %d\n", p);
      ...
      if(fabs(testnhat.x -Sys->Particles[p].EndPoint.x)>1e-3 || fabs(testnhat.y -Sys->Particles[p].EndPoint.y)>1e-3 || fabs(testnhat.z -Sys->Particles[p].EndPoint.z)>1e-3){
        printf("\nEndPoint Error!\n");Err=2;
      ...
      if(fabs(PhaseEnergy - Sys->Particles[p].PhaseFactorEnergy)>1e-3){
        printf("\nPhaseError!\n");Err=2;
    ...
    for(int index = p0+1; index<Sys->twomaxpop;index++){
      ...
      if(  deltaRsqr(CurrentParticle.MidPoint, PartnerParticle.MidPoint, Lx, Ly) 
        ...
        if ( CheckOverlap(PartnerParticle, CurrentParticle, Lx, Ly, &tmp) == ALOT) {
          printf("\n");
          PrintVec(CurrentParticle.Pos, "botpoint1");
          PrintVec(CurrentParticle.MidPoint, "midpoint1");
          PrintVec(CurrentParticle.EndPoint, "endpoint1");
          PrintVec(PartnerParticle.Pos, "botpoint2");
          PrintVec(PartnerParticle.MidPoint, "midpoint2");
          PrintVec(PartnerParticle.EndPoint, "endpoint2");
          printf("one: %d two: %d\n", p0, index);
          printf("sizep0: l %lf r %lf sizep1: l %lf r %lf\n", PartnerParticle.MyLength/2, PartnerParticle.MyRad, CurrentParticle.MyLength/2, CurrentParticle.MyRad);
          printf("proc cond: %d mutual dist: %lf\n", tmp, sqrt(deltaRsqr(CurrentParticle.MidPoint, PartnerParticle.MidPoint, Lx, Ly)));
          Err=3;
        }
      }
    }
...
int ErrorCheck_ParticleProps(sys *Sys){
  int p0, type, Err=0, ind=0;
  double tmp;
  
  for(p0=0;p0<Sys->twomaxpop;p0++){
    if(p0==Sys->maxpop)ind=1;
    if(p0<Sys->TotCurPop[ind]+ind*Sys->maxpop){
      if(p0<Sys->maxpop){
        tmp=BOT;
          if(fabs(Sys->Particles[p0].MeshId-tmp)>1e-10){
            printf("ERROR: particle %d should be BOT but MeshId is %d instead of %d\n",
                    p0, Sys->Particles[p0].MeshId, BOT
                  );Err=7;
          }
      }
        ...
        tmp=TOP;
          if(fabs(Sys->Particles[p0].MeshId-tmp)>1e-10){
            printf("ERROR: particle %d should be TOP but MeshId is %d instead of %d\n",
                    p0, Sys->Particles[p0].MeshId, TOP
                  );Err=7;
          }
      }
      
      type=Sys->Particles[p0].type;
      if(type==EMPTY){printf("ERROR: particle %d has type EMPTY\n", p0);Err=7;}
      else{
        tmp=Sys->BindPType[type];
        if(fabs(Sys->Particles[p0].MyIntStr-tmp)>1e-10){ 
          printf("ERROR: particle %d is type %d but IntStr is %lf instead of %lf\n",
                  p0, type, Sys->Particles[p0].MyIntStr, tmp
                );Err=7;
        }
        
        tmp=Sys->RanPType[type];
        if(fabs(Sys->Particles[p0].MyIntCutoff-tmp)>1e-10){
          printf("ERROR: particle %d is type %d but IntRange is %lf instead of %lf\n",
                  p0, type, Sys->Particles[p0].MyIntCutoff, tmp
                );Err=7;
        }
        
        tmp=Sys->RanPType[type]*Sys->RanPType[type];
        if(fabs(Sys->Particles[p0].MyIntCutoffsqr-tmp)>1e-10){
          printf("ERROR: particle %d is type %d but IntRanSqr is %lf instead of %lf\n",
                  p0, type, Sys->Particles[p0].MyIntCutoffsqr, tmp
                );Err=7;
        }
        
        tmp=Sys->DPType[ind][type];
        if(fabs(Sys->Particles[p0].MyDiam-tmp)>1e-10){
          printf("ERROR: particle %d is type %d but Diam is %lf instead of %lf\n",
                  p0, type, Sys->Particles[p0].MyDiam, tmp
                );Err=7;
        }
        
        tmp=Sys->DPType[ind][type]/2;
        if(fabs(Sys->Particles[p0].MyRad-tmp)>1e-10){
          printf("ERROR: particle %d is type %d but Rad is %lf instead of %lf\n",
                  p0, type, Sys->Particles[p0].MyRad, tmp
                );Err=7;
        }
        
        tmp=Sys->DPType[ind][type]*Sys->DPType[ind][type]/4;
        if(fabs(Sys->Particles[p0].MyRadSqr-tmp)>1e-10){
          printf("ERROR: particle %d is type %d but RadSqr is %lf instead of %lf\n",
                  p0, type, Sys->Particles[p0].MyRadSqr, tmp
                );Err=7;
        }
        if(fabs(Sys->Particles[p0].MeshParticleCutoffsqr-tmp)>1e-10){
          printf("ERROR: particle %d is type %d but MeshParticleCutoff is %lf instead of %lf\n",
                  p0, type, Sys->Particles[p0].MeshParticleCutoffsqr, tmp
                );Err=7;
        }
        
        tmp=Sys->LPType[ind][type];
        if(fabs(Sys->Particles[p0].MyLength-tmp)>1e-10){
          printf("ERROR: particle %d is type %d but length is %lf instead of %lf\n",
                  p0, type, Sys->Particles[p0].MyLength, tmp
                );Err=7;
        } 
      } 
    }
      ...
      type=Sys->Particles[p0].type;
      if(type!=EMPTY){printf("ERROR: particle %d is not -100 but %d despite being over current pop %d\n", p0, type, Sys->TotCurPop[ind]+ind*Sys->maxpop);Err=7;}
    }
  }
...
int ErrorCheck_ParticleMesh(sys *Sys){
  ...
  for(int p = 0; p<Sys->twomaxpop;p++){
    ...
    MPenergy = ParticleMeshEnergy(Sys, &(Sys->Particles[p]));
    if(MPenergy>1e-6){
      printf("\nERROR: Particle %d overlapping with mesh (%lf)\n", p, MPenergy);
      printf("meshId %s\n",(Sys->Particles[p].MeshId==TOP ? "Top":"Bot"));
      printf("Current MeshEnergy %lf\n", Sys->Particles[p].MeshParticleEnergy);
      PrintVec(Sys->Particles[p].Pos, "Pos");
      PrintVec(Sys->Particles[p].MidPoint, "MidPoint");
      PrintVec(Sys->Particles[p].EndPoint, "EndPoint");
      Err=8;
    }
  }
  return Err;
...
double MaxParticleCutoffSqr(particle P1, particle P2){
  return (P1.MyLength + P2.MyLength +  P1.MyDiam + P2.MyDiam)*
         (P1.MyLength + P2.MyLength +  P1.MyDiam + P2.MyDiam)/4; 
}
...
double ParticleMeshEnergy(sys *Sys, particle *MyParticle){
  ...
  diffvec.z -= MyPos.z;
  double MeshParticleEnergy = 0.0;
  if( MyMeshId*diffvec.z < 0.0 || diffvec.z*diffvec.z < MPCutoffsqr)  MeshParticleEnergy+= ALOT;
...
double CheckOverlap(particle Moved, particle Partner, double Lx, double Ly, int *loop){
  ...
  vec axperp_both_nhat, axperp_moved_nhat, r12, nhat_partner, tmp_vec;


  //Cap-Cap overlap
  if( Moved.type != Partner.type &&
    deltaRsqr(Moved.EndPoint, Partner.EndPoint, Lx, Ly)<(r_moved + r_partner)*(r_moved + r_partner) 
    ){*loop=6;return ALOT;}
    
  if(l_moved_on2 + l_partner_on2>1e-5){
    ...
    if (tmp<1e-5){
      ...
      if( fabs(tmp) > l_partner_on2 + l_moved_on2 + r_partner + r_moved ||
      ...
      /***********Check for Cap-Cylinder intersection************/
      threedvecdiff(&tmp_vec, Partner.EndPoint, Moved.MidPoint);
      tmp = dotprod(tmp_vec, Moved.nhat);tmp*=tmp;
      tmp = threed_NormSqr(tmp_vec) - tmp;
      if(Moved.type != Partner.type && tmp < (r_partner + r_moved)*(r_partner + r_moved)){*loop=7;return ALOT;}
      
      threedvecdiff(&tmp_vec, Moved.EndPoint, Partner.MidPoint);
      tmp = dotprod(tmp_vec, nhat_partner);tmp*=tmp;
      tmp = threed_NormSqr(tmp_vec) - tmp;
      if(Moved.type != Partner.type && tmp < (r_partner + r_moved)*(r_partner + r_moved)){*loop=7;return ALOT;}
      
      /***********Cylinder-Cylinder intersection************/
      if(l_partner_on2>1e-10 && l_moved_on2>1e-10){
      ...
      else{*loop=-7;return 0.0;}
    }
  }
  else{*loop=-6;return 0.0;}
}
  ...
  for(int i=0;i<9;i++){
    ...
    for(k=0;k<CellList->BotPopulations[Cx][Cy];k++){
      ...
      if(partner!=p0){
        ...
        if(   deltaRsqr(PartnerParticle.MidPoint, MovedParticle->MidPoint, Lx, Ly) 
            < MaxParticleCutoffSqr(PartnerParticle, *MovedParticle)
          ){
