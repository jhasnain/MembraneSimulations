...
int ErrorCheck_Etot(sys *Sys){
 if( fabs(Sys->TotEnergy - (Sys->MeshEnergy + Sys->BiasEnergy + Sys->TotVeff))>1e-6){
   printf("Error, Total energy not sum of Mesh, Bias, and Veff:\n");
   printf("  TotE stored: %lf\n", Sys->TotEnergy);
   printf("  SumE: %lf\n", (Sys->MeshEnergy + Sys->BiasEnergy + Sys->TotVeff));
   printf("  ME stored: %lf\n", Sys->MeshEnergy);
   printf("  BiasE stored: %lf\n", Sys->BiasEnergy );
   printf("  Veff stored: %lf\n", Sys->TotVeff);
   return 4;
  }
  else return 0;
}
...
void ErrorCheckConfiguration(sys *Sys, int mciteration, int acc){
  ...
  if(mciteration>0){ 
    ...
    TotErrs+=ParseError(ErrorCheck_Etot(Sys));
